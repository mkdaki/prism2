...
**テスト項目**:
  ...
  * [ ] 推移比較機能を実際に使用
  * [x] 第1回比較の実施（初回テスト・課題の洗い出し）
  * [ ] 改善タスクの実施（第1回結果を受けた改善）
  * [ ] 第2回以降の比較実施（改善後の検証）
  * [ ] シナリオ3-1: 不動産価格の推移
  * [ ] シナリオ3-2: 競合商品の価格変動
  * [x] シナリオ3-3: 求人市場のトレンド変化（任意）
...
【第1回比較】実施日: 2026/01/15
*E-2-1で準備したデータ（dataset_id=4, 9）を使用した初回テスト*
...
【改善タスク】実施期間: YYYY/MM/DD ～ YYYY/MM/DD

**位置づけ**: 第1回比較で判明した課題を改善し、第2回以降の比較で効果を検証する。
改善作業と並行して、週2-3回のデータ収集（スクレイピング）を継続する。

**改善の優先順位方針**:
- **最優先（実装必須）**: ビジネス価値に直結する分析内容の改善
- **高優先**: 分析の深さを向上させる機能
- **中優先**: UI/UXの改善
...
- Phase 1完了後、第2回比較を実施（最優先タスクの効果検証）
- Phase 2完了後、第3回比較を実施（全改善の効果検証）

---

【第2回比較】実施日: YYYY/MM/DD（改善後）
*改善タスク完了後、新たに収集したデータを使用*
...
- 実施回数: N回（改善前1回、改善後M回）
- 改善タスクの実施状況:
  - 実施した改善: [プロンプトv2、UI改善など]
  - 実施しなかった項目とその理由: [...]
- 改善前後での変化:
  - 改善前（第1回）: 継続利用しない。統計差分に偏り、価値なし
  - 改善後（第2回以降）: [評価がどう変わったか]
- 時間短縮効果: 平均X%削減
- 最も価値を感じた点: [...]
- 改善が必要な点: [残っている課題]
- 「キラー機能」として成立するか: Yes/No、理由: [...]
  - 改善前の評価: No（統計差分では価値なし）
  - 改善後の評価: [Yes/No、理由]
- 継続利用意向: [最終的な判断]
