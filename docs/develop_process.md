...
* [ ] データセット間の関連を管理するテーブル設計を決定
  ...
  * [ ] 案C: 比較はAPIレベルのみで、永続化しない（最小実装）
  
* [ ] 選択した設計の実装
  * [ ] Alembicマイグレーションファイルの作成
  * [ ] `app/models.py` へのモデル追加（必要な場合）
  * [ ] マイグレーション実行の確認

* [ ] テスト
  * [ ] マイグレーションの up/down が正常に動作すること
  * [ ] 外部キー制約が正しく設定されていること（CASCADE等）

**決定事項（ここに記録）**:
- [ ] 採用する設計案を決定したら、ここに記録する
...
* [ ] `GET /datasets/compare?base={dataset_id}&target={dataset_id}` エンドポイントを実装
  * [ ] base（基準データ）とtarget（比較対象データ）の存在チェック
  * [ ] 両データセットの stats を取得（B-1の処理を再利用）
  * [ ] 統計差分の計算
    * [ ] 行数の差分（増減数、増減率）
    * [ ] 数値カラムの差分（min/max/avg の変化）
    * [ ] 文字列カラムの差分（頻出値の変化、新規/消失カテゴリ）
...
* [ ] レスポンス形式を確定
  ...
  * [ ] `comparison`（差分サマリー）
    * [ ] `rows_change`（行数の変化）
    * [ ] `columns_change`（カラムごとの変化）
  
* [ ] エラーハンドリング
  * [ ] データセットが存在しない場合：404
  * [ ] カラム構成が大きく異なる場合：警告メッセージ（比較は継続）
  * [ ] 同じデータセットを指定した場合：400 Bad Request

* [ ] ユニットテスト
  * [ ] 正常系：2つのデータセット比較が成功
  * [ ] 異常系：存在しないIDで404
  * [ ] エッジケース：カラム構成が異なる場合の挙動
